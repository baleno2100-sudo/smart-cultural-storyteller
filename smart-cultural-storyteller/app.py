...
accent_color = "#FFA500"  # Orange color
# ===========================================
...
def generate_story_with_title(prompt, category):
    ...
    return clean_title(title), story, moral
...
def trigger_story_generation():
    if not st.session_state["prompt"].strip():
        ...
        with st.spinner("Summoning your story... 🌌"):
            ...
            # Save to database
            c.execute("INSERT INTO stories (title, category, story, moral) VALUES (?, ?, ?, ?)",
                      (title, category, story, moral))
            conn.commit()
...
c.execute("SELECT id, title FROM stories ORDER BY created_at DESC LIMIT 10")
rows = c.fetchall()

for row in rows:
    story_id, title = row
    expander_title = f"<span class='story-card-title'>{clean_title(title)}</span>"
